...
{
    class BackFactory : WorkoutFactory
    {
        ...
        public BackFactory(int currentWave, int currentWaveIndex, ExerciseList e)
        {
            Factory = new WorkoutExerciseListFactory(e.T1, e.T2, e.T3A, e.T3B, e.T4A, e.T4B);

            _currentWave = currentWave;
            _currentWaveIndex = currentWaveIndex;     
            _exerciseList = Factory.CreateExerciseList();
        }

       
        public override Workout GetWorkout()
        {
            return new Back(_currentWave, _currentWaveIndex, _exerciseList);
        }
